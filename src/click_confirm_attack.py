...
CONFIRM_ATTACK_TEMPLATE = PROJECT_ROOT / "screenshots" / "templates" / "confirm_attack_button.png"

# 1280x720 下，确认弹窗的下半部分区域。
SEARCH_REGION = (560, 400, 340, 150)
...
def find_template(screen):
    template = read_image(CONFIRM_ATTACK_TEMPLATE)
...
def save_debug(screen, match):
    ...
    output_path = output_dir / f"confirm_attack_{time.strftime('%Y%m%d_%H%M%S')}.png"
...
def main():
    ...
    print(f"确认攻击按钮匹配分数：{match.score:.3f}")
    print(f"确认攻击按钮中心点：({cx}, {cy})")
    print(f"调试图：{debug_path}")

    if match.score < MATCH_THRESHOLD:
        print("确认攻击按钮匹配分数低于阈值，暂不点击。")
        return
